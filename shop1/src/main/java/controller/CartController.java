...
public class CartController {
	...
	public ModelAndView add(Integer id, Integer quantity, HttpSession session) {
		...
		mav.addObject("message",item.getName() + " : " + quantity + "개 장바구니에 추가");
	 ...
	 * 주문 전 확인 페이지
	 * 1. 장바구니에 상품 존재.
	 * 		상품이 없는 경우 : 예외 발생. CartEmptyException 예외 발생
	 * 2. 로그인 상태여야 함.
	...
	/*	Cart cart = (Cart)session.getAttribute("CART");
		...
		if(loginUser == null) {
			throw new LoginException("로그인 하세요.", "../item/list");
		}
	*/
		return null; //view의 이름 리턴. null인 경우 url과 같은이름을 호출
					 // /WEB-INF/view/cart/checkout.jsp
	}
	/*
	 * 주문확정
	 * 1. 로그인, 장바구니 상품 존재해야함. => aop로 설정
	 * 2. 장바구니 상품 saleitem 테이블에 저장, 주문정보(sale) 테이블에 저장=>service.checkend
	 * 3. 장바구니 상품 제거.
